 ...
 public:
  ...
  int Run() {
    ...
    std::cout << "=== Example 1: Synchronous SELECT query ===" << std::endl;
    {
      ...
      auto status = mysql_proxy->Query(ctx, "SELECT 1 AS num, 'Hello' AS msg", &result);
    ...
    std::cout << "\n=== Example 3: Prepared statement ===" << std::endl;
    {
      ...
      auto status = mysql_proxy->PreparedExecute(ctx,
          "INSERT INTO test_users (name) VALUES (?)", params, &result);
      
      if (status.OK()) {
        std::cout << "Prepared insert succeeded! Insert ID: " << result.insert_id << std::endl;
        
        // Query using prepared statement
        std::vector<std::string> query_params = {"Bob"};
        status = mysql_proxy->PreparedExecute(ctx,
            "SELECT * FROM test_users WHERE name = ?", query_params, &result);
    ...
    std::cout << "\n=== Example 4: Asynchronous query ===" << std::endl;
    {
      ...
      auto future = mysql_proxy->AsyncQuery(ctx, "SELECT * FROM test_users");
    ...
    std::cout << "\n=== Example 5: Multiple queries ===" << std::endl;
    {
      ...
      // Query all users
      auto status = mysql_proxy->Query(ctx, "SELECT COUNT(*) AS total FROM test_users", &result);
